  ...
  for (const embeddingSource of embeddingSources) {
    ...
    try {
      ...
      if (!shouldRefresh && existingPage?.checksum === checksum) {
        const existingParentPage: any = existingPage?.parentPage
      ...
      // Create/update page record. Intentionally clear checksum until we yes this is a test
      ...
      for (const { slug, heading, content } of sections) {
        ...
        try {
          ...
          console.log(`[${path}] Generating embedding for section: ${slug}`)
          ...
          console.log(`[${path}] Inserting section into database: ${slug}`)
          ...
          if (insertPageSectionError) {
            console.error(`Error inserting section: ${slug}`, insertPageSectionError)
